...
const game = readlineSync.question("Your selection: ");
    switch (game) {
        case ('even-game'):
            runEngine(`Answer "yes" if the number is even, otherwise answer "no".`,evenGame)
            break
        case ('calc-game'):
        ...
        case ('progression'):
            ...
            break
        case ('prime'):
            runEngine(`Answer "yes" if given number is prime. Otherwise answer "no".`,prime)
            break
        default: break
    }
    const replayAnswer = readlineSync.question(`Do you want replay?  Answer "yes" or "no": `);
    if (replayAnswer==="yes"){
    gameSwitch()
}
